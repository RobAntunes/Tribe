...
## Diff and Code Management System

The Tribe extension includes a powerful diff and code management system that allows agents to propose changes to your codebase and for you to review, modify, and apply those changes. Key features include:

### Change Groups

Changes are organized into logical groups, each containing:
- Files to modify
- Files to create
- Files to delete

Each change group includes metadata such as the agent that proposed the changes, a title, description, and timestamp.

### Granular Diffs

The diff system breaks down changes into "hunks" - specific sections of files that have been modified. This allows for more precise review and understanding of the changes.

### Checkpoints

You can create checkpoints of your workspace at any time, allowing you to:
- Save the current state of your codebase
- Restore to a previous checkpoint
- View diffs between the current state and a checkpoint

### Annotations

The system supports adding annotations to code changes, enabling:
- Comments on specific files or lines of code
- Threaded discussions with replies
- Collaboration between humans and agents

### Commands

The following commands are available for managing code changes:

- `tribe.applyChanges`: Apply a set of changes to the workspace
- `tribe.acceptChangeGroup`: Accept all changes in a change group
- `tribe.rejectChangeGroup`: Reject all changes in a change group
- `tribe.acceptFile`: Accept changes for a specific file
- `tribe.rejectFile`: Reject changes for a specific file
- `tribe.modifyChange`: Modify the content of a pending change
- `tribe.requestExplanation`: Request an explanation for a specific file change
- `tribe.viewDiff`: View a diff between original and new content
- `tribe.createCheckpoint`: Create a checkpoint of the current workspace
- `tribe.restoreCheckpoint`: Restore the workspace to a checkpoint
- `tribe.viewCheckpointDiff`: View diff between a checkpoint and the current state

### UI Integration

The diff and code management system is fully integrated with the Tribe UI, providing:
