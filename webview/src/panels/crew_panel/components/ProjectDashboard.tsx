...
import { Clipboard, CheckSquare, Clock, AlertCircle, Plus, Search, Filter, X, Calendar } from 'lucide-react';
...
export const ProjectDashboard: React.FC<ProjectDashboardProps> = ({
  ...
  const handleCreateTask = () => {
    ...
    const newTaskObj: Task = {
      ...
      assignedTo: newTask.assignedTo || agents[0]?.id || '',
      priority: newTask.priority || 1,
      status: newTask.status as 'pending' | 'in_progress' | 'completed' | 'failed',
      createdAt: new Date().toISOString(),
    ...
    <div className="project-dashboard">
      ...
      {systemEnabled ? (
                    ...
                    >
                      ...
                      {agents.map(agent => (
                        <option key={agent.id} value={agent.id}>
                          {agent.name || agent.role} - {agent.short_description || agent.role}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="task-details-content">
                    ...
                    <div className="task-details-actions">
                      <div className="status-actions">
                        <button 
                          className={`status-button pending ${selectedTask.status === 'pending' ? 'active' : ''}`}
                        ...
                        <button 
                          className={`status-button in-progress ${selectedTask.status === 'in_progress' ? 'active' : ''}`}
                        ...
                        <button 
                          className={`status-button completed ${selectedTask.status === 'completed' ? 'active' : ''}`}
                        ...
                        <button 
                          className={`status-button failed ${selectedTask.status === 'failed' ? 'active' : ''}`}
                          onClick={() => handleUpdateTaskStatus(selectedTask.id, 'failed')}
                          ...
                          .map(task => (
                            ...
                            >
                              ...
                              <div className="task-card-footer">
                                <div className="task-assignee" title={getAgentDescription(task.assignedTo)}>
                                  <div className="assignee-avatar">
                                    {getAgentName(task.assignedTo)?.substring(0, 2) || 'NA'}
                                  </div>
        ...
        <div className="project-system-disabled">
          ...
          <p>Enable the project management system to track tasks, assign work to agents, and monitor progress.</p>
          <button 
