...
def test_two_members():
    """Test if the Lambda API responds for a two team members request with minimal details."""
    
    # Lambda API endpoint
    lambda_api_endpoint = "https://teqheaidyjmkjwkvkde65rfmo40epndv.lambda-url.eu-west-3.on.aws/"
    
    # Two team members request
    two_members_request = {
        "messages": [
            {
                "role": "system", 
            ...
            {
                "role": "user", 
                "content": """Create two team members for a software development project.
    ...
    logger.info("Sending two team members request to Lambda API...")
    start_time = time.time()
    
    try:
        response = requests.post(
            lambda_api_endpoint,
            json=two_members_request,
            timeout=30  # 30 second timeout for two members request
        )
...
if __name__ == "__main__":
    logger.info("Starting two team members test")
    result = test_two_members()
